...
Fleet::Fleet():x_(0), y_(0), dest_(NULL), speed_(0), lastTicks_(0), owner_(0)
{
}

//Regular constructor
Fleet::Fleet(const std::vector<int>& inships, Planet* begin, Planet* end):
  x_(begin->x() + (UNSCALED_PLANET_RADIUS * begin->size())),
  y_(begin->y() + (UNSCALED_PLANET_RADIUS * begin->size())),
  dest_(end),
  start_(begin),
...
void Fleet::display(SDL_Surface* screen, const SDL_Rect& camera)
{
  ...
  SDL_Rect rect = {Sint16(x_ - 10 - camera.x), Sint16(y_ - 10 - camera.y), 20, 20};
