...
class BuildingInstance
{
 public:
  ...
  int projectileTime() {return projectileTime_;}
  bool exists() {return type_ != NULL;}
  //Building variables
  SDL_Surface* rotation(float angle = -1, bool complete = true)
  {return type_->rotation(angle, complete);}
  std::string effect() {return type_->effect();}
  const std::string effect() const {return type_->effect();}
  int buildtime() {return type_->buildtime();}
  int buildtime() const {return type_->buildtime();}
  float cd() {return type_->cd();}
  float cd() const {return type_->cd();}
  int range() const {return type_->range();}
  bool canFire() {return projectileTime_ > type_->cd();}
  ...
  {type_->display(x, y, angle, screen, complete);}
